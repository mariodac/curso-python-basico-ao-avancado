...
while option != 0:
    ...
    if option == 1:
        ...
        if client_index-1 in [x for x in range(len(bank.clients))]:
            ...
            if type_account in [1, 2]:
                if type_account == 1:
                    ...
                    bank.clients[client_index-1]._account = account
                elif type_account == 2:
                    agency = choice(bank.agencies)
                    account_number = randint(1, 1000)
                    account = SavingAccounts(agency, account_number)
                    bank.clients[client_index-1]._account = account
                print(f'Dados de conta criada: {agency=} - {account_number=}')
        ...
        if client_index-1 in [x for x in range(len(bank.clients))]:
            if hasattr(bank.clients[client_index-1], '_account'):
                value = only_int(input('Digite o valor a depositar =>'))
                bank.clients[client_index-1]._account.cash_deposit(value)
            else:
                print('Depósito não realizado, cliente não possui conta')
        else:
            print('Cliente não encontrado')
    elif option == 4:
        for index, client_item in enumerate(bank.clients):
        ...
        if client_index-1 in [x for x in range(len(bank.clients))]:
            if hasattr(bank.clients[client_index-1], '_account'):
                value = only_int(input('Digite o valor a sacar =>'))
                bank.clients[client_index-1]._account.withdraw_money(value)
            else:
                print('Saque não realizado, cliente não possui conta')
        else:
            print('Cliente não encontrado')